...
public class IntegerFractionCalculatorUserView implements FractionCalculatorUserView<IntegerFraction> {
	...
	public IntegerFraction objectAction(Operation operation) {
		...
		case DIVIDE:
			...
			try {
				...
				result = new IntegerFraction(0, 1);
				System.out.println("Incorrect fraction for division (fraction can't have \"0\" in denominator), the returned result is fraction " + result.toShortString());
			}
			break;
		default:
			break;
	...
	public enum IntegerFractionOperation implements Operation {
		...
		private static final SortedSet<String> OPERATIONS_NAMES_SORTED_SET = new TreeSet<>(new NameOperationComparator());

		static {
			for (IntegerFractionOperation ifo: values()) {
				OPERATIONS_MAP.put(ifo.name, ifo);
				OPERATIONS_NAMES_SORTED_SET.add(ifo.getName());
			}
		}
		...
		public static SortedSet<String> getAllNames() {			
			return OPERATIONS_NAMES_SORTED_SET;
		}
		...
		private static class NameOperationComparator implements Comparator<String> {

			@Override
			public int compare(String name1, String name2) {
				IntegerFractionOperation o1 = getValueFromName(name1);
				IntegerFractionOperation o2 = getValueFromName(name2);
